    ...
    public class RuntimeCurveRenderer
    {
        ...
        public static Material LineMaterial
        {
            get
            {
                if (s_LineMaterial == null)
                {
                    ...
                    if (shader == null)
                    {
                        Debug.LogError("RuntimeCurveRenderer: Could not find 'Hidden/Internal-Colored' shader.");
                        return null;
                    }
            ...
            float lineWidth = 2f)
        {
            ...
            for (int i = 0; i <= sampleCount; i++)
            {
                ...
                Vector2 viewPos = DrawingToView(new Vector2(t, v), curveArea, shownAreaMin, shownAreaMax);
            ...
            if (curve.length >= 2)
            {
                ...
                {
                    DrawWrapRegion(curve, curveArea, shownAreaMin, shownAreaMax, wrapColor,
                        visibleMinTime, firstKeyTime, sampleCount / 4);
                }
                ...
                {
                    DrawWrapRegion(curve, curveArea, shownAreaMin, shownAreaMax, wrapColor,
                        lastKeyTime, visibleMaxTime, sampleCount / 4);
                }
            }
        ...
        public static void DrawMiniCurve(AnimationCurve curve, Rect rect, Color color)
        {
            ...
            if (timeRange < 0.001f) { timeRange = 1f; minTime -= 0.5f; }
            if (valRange < 0.001f) { valRange = 1f; minVal -= 0.5f; }

            Vector2 areaMin = new Vector2(minTime - timeRange * padding, minVal - valRange * padding);
            Vector2 areaMax = new Vector2(maxTime + timeRange * padding, maxVal + valRange * padding);
        ...
        public static Vector2 DrawingToView(Vector2 drawingPoint, Rect curveArea, Vector2 shownAreaMin, Vector2 shownAreaMax)
        {
            ...
            float y = curveArea.y + curveArea.height - (drawingPoint.y - shownAreaMin.y) / rangeY * curveArea.height;
            return new Vector2(x, y);
        }

        public static Vector2 ViewToDrawing(Vector2 viewPoint, Rect curveArea, Vector2 shownAreaMin, Vector2 shownAreaMax)
        {
            float rangeX = shownAreaMax.x - shownAreaMin.x;
            float rangeY = shownAreaMax.y - shownAreaMin.y;
            if (rangeX == 0f) rangeX = 1f;
            ...
            float y = shownAreaMin.y + (curveArea.y + curveArea.height - viewPoint.y) / curveArea.height * rangeY;
            return new Vector2(x, y);
        }

        public static void GetCurveBounds(AnimationCurve curve, out float minTime, out float maxTime, out float minVal, out float maxVal)
        {
            if (curve.length == 0)
            {
                minTime = 0f; maxTime = 1f;
                minVal = 0f; maxVal = 1f;
                return;
            }
            ...
            float timeRange = maxTime - minTime;
            if (timeRange < 0.0001f) timeRange = 1f;
